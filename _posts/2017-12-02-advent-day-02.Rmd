...
categories: ["Hacking"]
tags: ["advent of code"]
excerpt: "Solution for Day 2 of Advent of Code"
---

* [Problem](http://adventofcode.com/2017/day/2)
...
get_checksum <- function(x) {
    x <- read.table(text = x)
    x_rg <- apply(x, 1, function(x) diff(range(x)))
    sum(x_rg)    
}

get_checksum(spreadsheet)

get_checksum2 <- function(x) {
    x <- read.table(text = x)
    res <- apply(x, 1, function(r) {
        row_res <- lapply(r, function(.r) {
            bb <- r[.r%%r == 0]
            if (length(bb)== 2)
                max(bb)/min(bb)
            else NULL
        })
        pos <- !vapply(row_res, is.null, logical(1))
        stopifnot(sum(pos) == 1L)
        unlist(row_res[pos])
    })
    sum(res)
}

get_checksum2(spreadsheet)
